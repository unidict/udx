...
static inline udx_value_address_t udx_addr_make(uint32_t chunk_index, uint16_t offset) {
    return ((uint64_t)chunk_index << 16) | offset;
}

// Get chunk_index from address
static inline uint32_t udx_addr_get_chunk(udx_value_address_t addr) {
    return (uint32_t)(addr >> 16);
}

// Get offset from address (same for both formats)
static inline uint16_t udx_addr_get_offset(udx_value_address_t addr) {
...
udx_value_address_t udx_chunk_writer_add_block(udx_chunk_writer *writer,
    ...
    udx_value_address_t address = udx_addr_make(writer->current_chunk_index,
