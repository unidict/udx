...
- **Address Encoding**: 32-bit chunk index + 16-bit offset in chunk
...
- **Core Modules**: The foundational components that implement the UDX format specification. `udx_chunk` manages compressed data storage, `udx_keys` provides ordered key container functionality during database building, and `udx_types` defines all core data structures and serialization routines.
...
graph TB
    ...
    class Chunk,Keys,Types coreStyle
...
2. **Database Building**: `udx_db_builder` collects entries, storing data in chunks via `udx_chunk_writer` and indexing keys via `udx_keys`.
3. **Index Construction**: During the build phase, `udx_keys` uses a dynamic B-tree (`udx_btree`) to efficiently collect and sort entries out-of-order.
...
3. **Index Lookup**: The B+ tree index enables fast O(log n) key lookups with prefix matching support, returning chunk addresses without loading data.
...
|-----------|---------|---------|
| **B-tree** | `udx_keys` (internal) | Memory storage during **building phase** - dynamic, auto-sorted, supports insertions |
| **B+ tree** | UDX file index | Disk storage in **file format** - static, optimized for reading |

During **write**, entries are added to a dynamic B-tree (`udx_keys`) which efficiently handles out-of-order insertions. At the end, this B-tree is traversed in sorted order and converted to a **static B+ tree** that's written to disk. The static B+ tree format enables zero-copy parsing and efficient lookups when reading UDX files.
...
The library uses **key folding** (converting to lowercase) for indexing while preserving the original key case. This enables:
