...
void test_chunk_writer_add_single_block(void) {
    ...
    udx_value_address addr = udx_chunk_writer_add_block(writer, data, (uint32_t)(sizeof(data) - 1));
...
void test_chunk_writer_add_multiple_blocks(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        udx_value_address addr = udx_chunk_writer_add_block(writer, data, (uint32_t)sizeof(data));
...
void test_chunk_writer_add_null_data(void) {
    ...
    udx_value_address addr = udx_chunk_writer_add_block(writer, NULL, 100);
...
void test_chunk_writer_add_zero_size(void) {
    ...
    uint8_t data[] = "test";
    udx_value_address addr = udx_chunk_writer_add_block(writer, data, 0);
...
void test_chunk_writer_large_block(void) {
    ...
    udx_value_address addr = udx_chunk_writer_add_block(writer, large_data, 100000);
...
void test_chunk_reader_get_block(void) {
    ...
    udx_value_address addr = udx_chunk_writer_add_block(
...
void test_chunk_roundtrip_binary_data(void) {
    ...
    udx_value_address addr = udx_chunk_writer_add_block(writer, binary_data, (uint32_t)sizeof(binary_data));
    uint64_t table_offset = udx_chunk_writer_finish(writer);
    udx_chunk_writer_close(writer);
    fclose(file);
